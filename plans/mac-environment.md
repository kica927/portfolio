...
| **D** | **라즈베리파이 5** (arm64 Linux, 실물 보유) | RoboSec 실험 대상 | 실기 |
...
## 라즈베리파이 5 를 실습 대상으로 쓸 때

RoboSec 단계에서 Pi 가 실험 대상(SUT)이 됩니다. **이미 갖고 계신 arm64 Linux
실기**라는 것이 큰 이점입니다 — 대부분의 학생은 이런 대상이 없어 문헌의 예제를
씁니다.

다만 **로봇 운용 규칙과 충돌하지 않게** 하세요.
...
**하드웨어 없이 맥에서(A) 그대로 fuzzing 할 수 있습니다.** 실기(D)는 마지막에만
필요합니다 — 이 순서를 지키면 로봇을 거의 안 씁니다.
