...
여유 15시간(3%)입니다. **빡빡합니다.** 그래서 아래를 잘랐습니다.
...
| **하드웨어 없이** `domain/task/motion.py` · `baseline_mission.py` 를 대상으로 첫 fuzzing | A |

> **핵심: 1월에는 로봇을 거의 쓰지 않습니다.** 도메인 코드가 순수 파이썬이고
...
**A 에서 끝까지 밀고, 실기(D)는 마지막 검증에만 씁니다.**

| 실험 | 질문 | 환경 |
|---|---|---|
| E1 | 손상 페이로드를 다량 주입하면 정상 명령의 실효 수신율은? | A |
| E2 | 유효 형식 명령을 정상보다 자주 주입하면 — 클램프는 크기만 자르는데 **방향**은? | A → D |
| E3 | 이전 GRASP 명령을 나중에 재전송하면 상태 게이팅이 막는가? | A → D |
| E4 | 링크를 완전히 침묵시키면 워치독 발동까지 실제 몇 초, 그동안 몇 mm? | D |

> ⚠️ **E2·E4 는 차량이 움직입니다.** 바퀴를 띄우고 먼저 하세요. 속도 클램프를
> 시험하는 실험이 클램프가 깨진 상태에서 돌면 로봇이 실제로 튀어나갑니다.
> 실험망은 격리하고, 캘리브레이션 파일은 건드리지 않으며, 실험 후
> `perception_node` 를 되살립니다.
