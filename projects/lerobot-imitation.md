...
2. ~~ACT vs SmolVLA 를 같은 데이터·같은 에피소드 수로 비교~~ → **완료(오프라인
   지표로).** redball 데이터셋으로 ACT 를 새로 학습해 SmolVLA 와 나란히 두고
   loss 수렴·학습 비용·추론 지연을 비교했다. 실기 성공률 비교는 여전히 못
   한다(하드웨어 종료). [ACT vs SmolVLA — 오프라인 비교](act-vs-smolvla-offline.md)
