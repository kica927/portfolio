...
- **F1(NaN 누출) — 발견 + 패치 + 병합.** 배포본에 존재(19/21), 패치(비유한값→정지)
  후 21/21. 2026-08-30 저녁 `74b0cad`/`55ebab4`로 baseline 자체에 반영돼 team
  origin 에 PR #45 로 merge 됐다 — 지금 HEAD 기준으로 돌리면 **21/21**, F1 항목도
  PASS 다. 실측 bag 유한성 위반 0. → [취약점 리포트](../plans/robosec/threat_model.md)
- **F2(스푸핑·재전송) — 실기 확증, 방어는 별도 저장소에서 완성.** 송신자 인증·
  시퀀스가 없어, 유효 상태를 스푸핑하거나 낡은 명령을 재전송하면 FSM 이 실제로
  움직인다. 상태 게이팅(D4)은 **불법 전이만** 막을 뿐 합법 위장은 못 막는다. 이
  결함의 방어(HMAC+시퀀스)는 [udp-network-lab](udp-network-lab.md)에서 설계·구현·
  검증까지 끝났다(before/after 실증 2/2→0/2, 퍼징 3종 무결점). 다만 grippers 본체에
  수신 어댑터로 통합하는 것은 아직 안 됐다 — RoboSec 이 grippers 를 직접 수정하지
  않는다는 원칙상 이 통합은 이 저장소의 범위 밖이다.
- **하류 방어 심층 발견.** F1·F2 로 샌 값이 `/cmd_vel` 까지 갔지만 실물 바퀴는 안
  돌았다 — 하류(기구학/STM32)가 삼켰다. 단 이는 **우연한 보호**이지 설계된 방어가
  아니므로 도메인에서 고쳐야 한다.
- **체커 자체의 결함 발견(2026-09-06).** 일반 통주행 녹화 2건(9분·image/points
  제외)을 스캔했더니 위반이 1258·1382건 나왔다 — 실제로는 거의 다 오탐이었다.
  `ROTATION_CAP`이 0.25로 낡아 있던 탓이었다(같은 날 아침 grippers 커밋 `86bea84`로
  실제 회전 한계가 0.6으로 이미 올라가 있었다: 0.25는 데드밴드 아래라 실제로
  안 도는 값이었다는 IMU 실측 근거까지 있었다). 0.6으로 고치니 위반이 1218·1334건
  줄었다. 남은 40·48건은 GRASP 좌우 정렬의 미세 전진(lin=0.100, az=0.000, 8개씩
  클러스터)으로 일관돼 있어 **불변식 I1이 실제 설계보다 엄격했던 것**임을 확인,
  "GRASP 중 순수 전진은 예외"로 완화했다(selftest 13개로 확장, 전부 통과). 재검사
  결과 두 녹화 다 위반 0건. **모델(불변식)도 실물처럼 계속 재검증해야 한다는
  것을 스캐너 자신에게 적용해 본 사례다.**
...
- 불변식은 I1·I2·I3·I5·I6·F 다. I4·I7·I8 은 실측 근거가 더 필요하다(I4는 명령
  수신 시각이 있어야 판정 가능한데 `/cmd_vel` 녹화엔 그 시각이 없다 — 하드웨어
  워치독 계측이 필요해 오프라인 스캐너 범위 밖이다).

## Future Work

- [udp-network-lab](udp-network-lab.md) 로 F2 를 닫고, 녹화 bag 재생으로 before/after 확증(완료).
- 스캐너에 I4 추가(I3는 2026-09-06 이전에 이미 구현돼 있었음 — 정상 궤적 오라클은
  2026-08-31 확보, 이후 검증은 소프트웨어로 가능).
