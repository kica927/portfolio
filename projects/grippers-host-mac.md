...
| **미션 FSM 확장 (2026-09 초~)** | 이식 이후 이 저장소가 실제 라이브 Host 스택이 됐습니다. INSERT를 Pi 쪽 LiDAR 좌우허용치에서 Host 쪽 120도 부채꼴 목표영역 게이트로 재설계(`basket_target.py`, LiDAR 없이 완결), GRASP_ALIGN 재정렬 로직(BACK_OFF+좌우 스윕, 미수렴 시 후진 전환), `manual_insert_probe.py`(WASD로 INSERT 경계를 직접 시험하는 수동 도구) 추가 |
...
**두 번째 사례 — 넣었다가 도로 뺀 기능.** `AWAIT_CONTINUE`(라벨 그룹이 화면에서
소진되면 계속/정지를 사용자에게 물어보는 기능)를 이식했으나(`fc74339`), 2026-09-04~05
실기에서 도로 제거했습니다(`317d773`). 그룹 소진 판정 자체가 검출 노이즈에 취약해
멀쩡한 실행 중간에 불필요하게 멈춰 세웠기 때문입니다. **기능을 넣는 것과 실기에서
버티는 것은 다른 문제**라는 걸 보여주는 사례라 코드에서 지웠지만 기록은 남깁니다.

---

## Problems & Debugging — 실기에서 잡은 결함

**GridPathPlanner 가 "제자리에서 이상하게 돈다".**

```
Problem     CARRY_TO_DEST 등에서 로봇이 실제로는 거의 안 움직였는데 큰 호를
            그리며 도는 것처럼 보인다
Symptom     실기 로그: 로봇 좌표는 5cm 이내로 거의 안 움직였는데 yaw는 135도
            넘게 돌았다가 29도 되돌아옴
Hypothesis  경로계획기가 회전 관성(coast)으로 인한 미세한 위치 흔들림만으로도
            매 사이클 경로를 다시 짜는 것 아닌가
Root Cause  GridPathPlanner 는 매 사이클 경로를 처음부터 다시 짜고 회전량은
            비용에 안 넣는다. 로봇이 ROTATE 중이라 실제로는 거의 안 움직였는데도
            근처 장애물 좌/우 우회 비용이 비슷하면 mm 단위 위치 흔들림만으로
            sub_goal 이 뒤집힌다. DriveSequencer 가 회전 관성으로 STOP↔ROTATE 를
            오갈 때마다 이 흔들리는 sub_goal 로 목표각을 다시 잠그면 실제 필요한
            것보다 훨씬 큰 호를 그리게 된다
Fix         GridPathPlanner 가 마지막 실제 계산 위치를 기억해 두고, 거기서
            PATH_REPLAN_MIN_MOVE_M(5mm) 이내로만 움직였으면 다시 계산하지 않고
            직전 결과를 그대로 돌려준다. DriveSequencer 의 모드 이름으로는 얼리지
            않는다 — 그 이름과 "실제로 유의미하게 움직였는가"가 대응하지 않아서다
Verification tests/test_grid_path_planner_freeze.py 신규 3개 + 전체 스위트
            201 passed 4 skipped. 첫 시도("DriveSequencer 모드로 얼리기")는
            정상적인 다음 부분목표 진행 시점의 STOP 사이클까지 잘못 얼려
            기존 테스트를 깨뜨려 폐기
Result      회전은 정의상 제자리라 문턱 안에 저절로 들어와 얼어붙고, 실제 전진
            (한 사이클 7~14mm)은 문턱보다 뚜렷이 커서 사실상 매 사이클 재계산됨
```
...
- ✅ **(해소) 실기 통합** — 이 문서 초판 시점엔 `--mock-complete` 로 차량 없이
  FSM 전이까지만 확인했다고 적었으나, 그 뒤(2026-09 초) 실제 로봇과 반복 통합해
  다수의 실주행을 확인했습니다(위 Problems & Debugging·What Failed 참고).
- 🟡 **원인 미상의 무반응 사건(2026-09-05 밤)** — 12초 넘게 회전 명령을 계속
  보냈는데 로봇이 전혀 안 움직인 사건이 있었습니다. Host→Pi→ROS2 명령 전달
  경로는 로그상 전부 정상이었고, 그날 낮의 servo1·GridPathPlanner 변경과도
  연결점이 없다고 판단했습니다. Pi 전체 재기동으로 회복은 확인했지만 **왜
  무반응이었는지는 로그만으로 확정하지 못했습니다** — 배터리·기구적 걸림·STM32
  전원 쪽을 의심하는 선에서 재발 대비용으로 로그만 보존해 뒀습니다.
