...
| | | |
|---|---|---|
| ![](../assets/perception/synth_0054.jpg) | ![](../assets/perception/synth_0127.jpg) | ![](../assets/perception/synth_0143.jpg) |
| `YoloSrc.0054` | `YoloSrc.0127` | `YoloSrc.0143` |

**같은 씬, 다른 조건.** 세 프레임의 바닥재(밝은 오크 / 어두운 월넛 / 창백한 애쉬),
조명 세기, 벽 너머로 보이는 배경, 물체 구성이 전부 다르다. 파일명이 연번
(`YoloSrc.####`)인 것이 핵심이다 — 사진 몇 장이 아니라 **렌더 시퀀스**다.

이것이 도메인 랜덤화다. 모델이 "흰 물체"를 나무 무늬나 조명 색이 아니라 **모양**으로
붙잡게 만들려면, 그 나머지를 전부 흔들어야 한다.

렌더된 씬에는 바닥 4모서리의 ArUco 마커까지 포함되어 있다 — 좌표계 검출도 같은
데이터로 함께 준비된다. 로봇 자신도 씬 안에 있어서, 로봇이 물체를 가리는 상황이
학습 데이터에 자연히 들어간다.
...
| | |
|---|---|
| ![탑뷰 검출](../assets/perception/topview_detection.jpg) | ![ArUco](../assets/perception/topview_aruco.jpg) |
| 단일 클래스 검출 — `rook 95%` | ArUco 5개: 바닥 4 + **로봇 1** |
...
![탑뷰 HUD](../assets/perception/topview_hud.jpg)

2대 동시 실행 화면. 두 카메라 모두 6클래스를 82~96% 로 잡고 있고, HUD 는
`calib=APPROX  floor=4/4  reproj=2.14px / 2.32px  [LOCKED]` 를 띄운다.

주목할 줄은 아래 두 개다.

```
x=888mm  y=315mm  yaw=83.8deg  cams=1
OUT OF WORKSPACE (blind strip near wall)
```

`cams=1` — 카메라가 2대인데 **1대만** 로봇 마커를 보고 있다. 그리고 그 자세는
선언된 작업영역 밖, 벽에 붙은 사각지대다. 중요한 것은 이것이 임시로 찍어 본 문구가
아니라 **`run_localize.py` 의 정식 상태 표시**라는 점이다 — 자세 추정이 성공했는데도
(`pose.ok`) 그 값을 쓰면 안 되는 구간에 이름을 붙여 놓았다.

`ROBOT LOST`(못 봄)와 `OUT OF WORKSPACE`(봤지만 믿을 자리가 아님)를 **다른 상태로**
가른 것이 요점이다. 둘을 뭉뚱그리면 "값이 나왔으니 쓴다"가 된다.

### 사각지대를 스팟 체크로 확인하지 않는다

`selftest.py` 는 이 사각지대를 **전수로** 증명한다.

```
선언한 작업영역을 37 × 29 = 1,073 점 격자로 자른다
각 점에 로봇 마커를 가상 배치해 두 카메라로 투영한다
2대 중 하나라도 마커를 보는가?
→ 하나도 못 보는 점이 있으면 실패
```

카메라를 몇 군데 옮겨 보고 "잘 보이네" 하는 것과 다르다. **작업영역이라고 선언한
집합 전체에 대해** 커버리지가 성립함을 확인한다. 같은 파일에 바구니가 작업영역과
겹치지 않는지, 가로 1.8 m 가 프레임에 들어오는지도 같은 방식으로 들어 있다.

> 이것이 RoboSec 의 출발점이 된 사고방식입니다 — **속성을 문장으로 적고, 입력
> 공간 전체에 대해 반증을 찾는다.** 크래시가 아니라 위반을 찾는 것이죠.
...
- **`OUT OF WORKSPACE` 는 표시만 하고 막지는 않는다.** 화면에 상태는 뜨지만, 그 자세를
  받은 상위 로직이 실제로 명령을 보류하는지는 코드로 확인하지 않았다.
